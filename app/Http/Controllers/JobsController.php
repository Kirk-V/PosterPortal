...
class JobsController extends Controller
{
    ...
    public function getJobsData($page, $entriesPerPage = 50)
    {
        $jobs = DB::table('Posters')->where('state', 'accepted')
            ...
            ->select('Posters.*', 'Requests.*', 'Transactions.*', 'jobs.poster_id', 'jobs.job_state as job_state', 'jobs.technician', 'jobs.print_date', 'jobs.job_id', 'jobs.emailed_receipt_req','jobs.emailed_receipt_grant_holder','jobs.emailed_receipt_ssts', 'courses.number', 'courses.department as course_department')->skip(($page - 1) * $entriesPerPage)->take($entriesPerPage)->get([]);
